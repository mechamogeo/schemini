...
describe("Phone Validator", () => {
  ...
  describe("formatPhone", () => {
    it("should format to E.164 by default", () => {
      ...
      expect(
        formatPhone("(415) 555-2671", "E.164", { defaultCountry: "US" }),
      ).toBe("+14155552671");
    });
    ...
    it("should format to INTERNATIONAL", () => {
      expect(formatPhone("+14155552671", "INTERNATIONAL")).toBe(
        "+1 415 555 2671",
      );
    });
  ...
  describe("s.string().phone()", () => {
    ...
    it("should return invalid_string error with phone format", () => {
      const schema = s.string().phone();
      ...
      if (!result.success) {
        ...
        expect(result.error.issues[0]?.expected).toBe("phone");
      }
    });
