...
describe("Currency Validator", () => {
  describe("parseCurrency", () => {
    ...
    describe("min/max constraints", () => {
      ...
      it("should work with both min and max", () => {
        ...
        expect(parseCurrency("$150.00", { min: 10, max: 100 }).valid).toBe(
          false,
        );
      });
    });
  ...
  describe("s.string().currency()", () => {
    ...
    it("should return invalid_string error with currency format", () => {
      const schema = s.string().currency();
      ...
      if (!result.success) {
        ...
        expect(result.error.issues[0]?.expected).toBe("currency");
      }
    });
